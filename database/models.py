...
class Database:
        ...
        with self.conn.cursor() as cursor:
            ...
            try:
                ...
                cursor.execute('''
                    ...
                    pupil_notifications JSONB,
                    teacher_notifications JSONB
                );''')
    ...
    # status 
    def update_pupil_status(self, pupil_id: int, status: str):
        with self.conn.cursor() as cursor:
            cursor.execute('''UPDATE public.pupils SET status = %s WHERE pupil_id = %s;''',
                           (status, pupil_id))
            self.conn.commit()

    def update_teacher_status(self, teacher_id: int, status: str):
        with self.conn.cursor() as cursor:
            cursor.execute('''UPDATE public.teachers SET status = %s WHERE teacher_id = %s;''',
                           (status, teacher_id))
            self.conn.commit()
    ...
    def get_admin(self, admin_id: int) -> bool:
        with self.conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:
            cursor.execute(
                "SELECT * FROM public.admins WHERE admin_id = %s",
                (admin_id,)
            )
            row = cursor.fetchone()
    ...
    def set_teacher_notification(
        ...
        """
        Adds or updates one entry in the teacher_notifications JSONB
        for this admin. The key is the teacher chat_id (as string), the value
        is a two-element array [header_id, body_id].
        """
        with self.conn.cursor() as cur:
            cur.execute(
                """
                ...
                SET teacher_notifications =
                  ...
                  -- concatenate (overwriting any existing key)
                  || %s
                WHERE admin_id = %s
                """,
                (
                    # Json adapter will cast our dict to SQL literal JSONB
                    Json({ str(teacher_chat_id): [header_id, body_id] }),
                    admin_id
                )
            )
        # if you aren’t in autocommit mode:
        self.conn.commit()
    def set_pupil_notification(
        self,
        admin_id: int,
        pupil_chat_id: int,
        header_id: int,
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                ...
                """,
                (
                    Json({ str(pupil_chat_id): [header_id, body_id] }),
                    admin_id
                )
            )
        self.conn.commit()

    def get_teacher_notifications(self, admin_id: int) -> dict[str, list[int]]:
        """
        Returns the JSON object as a Python dict:
           { "123456789": [hdr_id, body_id], … }
        """
        with self.conn.cursor() as cur:
            cur.execute(
            ...
            row = cur.fetchone()
        return row[0] or {}  # row[0] is already a Python dict if psycopg2-json enabled

    def clear_teacher_notifications(self, admin_id: int):
        with self.conn.cursor() as cur:
            cur.execute(
              "UPDATE public.admins SET teacher_notifications = '{}'::jsonb WHERE admin_id = %s",
              (admin_id,)
            )
        self.conn.commit()

    # and similarly for pupils:
    def get_pupil_notifications(self, admin_id: int) -> dict[str, list[int]]:
        with self.conn.cursor() as cur:
            cur.execute(
    ...
    def clear_pupil_notifications(self, admin_id: int):
        with self.conn.cursor() as cur:
            cur.execute(
              "UPDATE public.admins SET pupil_notifications = '{}'::jsonb WHERE admin_id = %s",
              (admin_id,)
            )
        self.conn.commit()
